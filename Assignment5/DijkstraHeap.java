public class DijkstraHeap {
	 ...
	 * @return The node of the highest priority (lowest distance)
	 */
	...
	public void add(DijkstraNode node) {
		...
		heap[nextLeaf] = node;				// Adds the node to the next available leaf position
	...
	public DijkstraNode popMin() {
		...
		heap[0] = heap[nextLeaf];			// Overwrites the node with the highest priority
	...
	private void floatRecurse(int parentIndex, int insertedIndex) {
		...
		if (parentNode.getDistance() > insertedNode.getDistance()) {			// Swaps the parent and current nodes if the parent's distance is greater than the child's
	...
	private void sinkNode(int parentIndex) {
		...
		else if (leftNode.getDistance() <= rightNode.getDistance()) {		// Sets the compared index to the left child if its distance is less than or equal to the right child's
		...
		if (parentNode.getDistance() > childNode.getDistance()) {			// Swaps nodes if the parent's distance is greater than the child's
