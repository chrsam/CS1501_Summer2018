 ...
 *  Execution:    for compression, add "r" or "n" as additional args to reset the dictionary 
...
public class LZWmod {
    ...
    public static void compress() {
        ...
        while (input.length() > 0) {
            ...
            if (t < input.length() && code < L) {    // Add s to symbol table.
                ...
                System.err.println("N Compression Ratio: " + oldCompressionRatio);
                ...
                if(W < MAX_BIT_WIDTH){ //We can still add words to the codebook without it being full
                    ...
                    if(Mode == 'r'){ //Reset mode
                        ...
                        // System.err.println("R Compression Ratio: " + newCompressionRatio);
    ...
    public static void expand() {
        ...
        if(Mode != 'n' && Mode != 'r') throw new IllegalArgumentException("This file contains an invalid compression mode: " + Mode);
        ...
        while (true) {
            ...
            if(i >= L){
                if(W < MAX_BIT_WIDTH){ //There is still room to expand the codebook
                    L = (int) Math.pow(2, ++W);
                    decompressionRatio = (double) uncompressedDataSize/compressedDataSize;
        ...
        System.err.println("Compression Ratio: " + decompressionRatio);
        BinaryStdOut.close();
    }

    public static void main(String[] args) {
        if(args.length >= 2){ //There is a mode as well as compress/expand option (used below)
            if(args[1].length() == 1 && (args[1].equals("n") || args[1].equals("r"))) {
